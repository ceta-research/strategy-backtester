#!/usr/bin/env python3
"""Run EOD Technical with exact ATO_Simulator params on Kite data."""
...
class KiteDataProvider:
    ...
    def fetch_ohlcv(self, exchanges, symbols=None, start_epoch=None, end_epoch=None, prefetch_days=400):
        ...
        sql = f"""SELECT date_epoch, open, high, low, close,
                  ...
                  FROM kite.kite_historical_day WHERE {where} ORDER BY symbol, date_epoch"""
...
if __name__ == "__main__":
    ...
    results = run_pipeline(config_path, data_provider=KiteDataProvider())

    print(f"\n{'='*80}")
    print("RESULTS (Kite data, ATO_Simulator matching config)")
    print(f"{'='*80}")
    for r in results:
