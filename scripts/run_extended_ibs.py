#!/usr/bin/env python3
"""Run Extended IBS Mean Reversion across all data sources.

Usage:
    python scripts/run_extended_ibs.py                        # all variants
    python scripts/run_extended_ibs.py --variant us           # US only
    python scripts/run_extended_ibs.py --variant nse_fmp      # NSE via FMP
    python scripts/run_extended_ibs.py --variant nse_native   # NSE via nse_charting_day
    python scripts/run_extended_ibs.py --variant nse_kite     # NSE via Kite parquet
"""
...
STRATEGIES_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "strategies", "extended_ibs")
...
class NseChartingDataProvider:
    ...
    def fetch_ohlcv(self, exchanges, symbols=None, start_epoch=None, end_epoch=None, prefetch_days=400):
        ...
        df = df.with_columns([
            pl.col("symbol").cast(pl.Utf8), pl.lit("NSE").alias("exchange"),
...
class LocalKiteDataProvider:
    def fetch_ohlcv(self, exchanges, symbols=None, start_epoch=None, end_epoch=None, prefetch_days=400):
        ...
        df = df.with_columns([pl.col("symbol").cast(pl.Utf8), pl.lit("NSE").alias("exchange")])
        df = df.with_columns((pl.lit("NSE:") + pl.col("symbol")).alias("instrument"))
        df = df.with_columns([
            pl.col("date_epoch").cast(pl.Int64), pl.col("open").cast(pl.Float64),
            pl.col("high").cast(pl.Float64), pl.col("low").cast(pl.Float64),
            pl.col("close").cast(pl.Float64), pl.col("average_price").cast(pl.Float64),
...
def run_variant(name, variant):
    print(f"\n{'='*80}")
    print(f"  Extended IBS Mean Reversion: {variant['label']}")
    print(f"{'='*80}")
    ...
    return {
        ...
        "cagr": best.get("cagr"), "max_drawdown": best.get("max_drawdown"),
        "calmar_ratio": best.get("calmar_ratio"), "sharpe_ratio": best.get("sharpe_ratio"),
        "sortino_ratio": best.get("sortino_ratio"),
        "num_configs": len(results), "elapsed": elapsed,
    }
...
def main():
    ...
    for name in variants_to_run:
        summaries.append(run_variant(name, VARIANTS[name]))
    print(f"\n{'='*80}")
    print("  Extended IBS Mean Reversion: Cross-Data-Source Comparison")
    print(f"{'='*80}")
