...
|---|---|---|---|
| 2026-05-03 | EOD-B DD-reduction research + convex ensemble + quiet-flip filter | `0ffef69`, `(convex)`, `141a41a` | (1) **DD-reduction research on EOD-B IR-hyst (22.26%/-16.49%/1.350):** tested cooldown after regime flip (re-entries are profitable, would HURT), DD-based entry skip (deep-DD entries are MOST profitable, would HURT), flip-frequency correlation (events↔return -0.51, events↔MDD -0.64, real signal but lagging filter doesn't help). (2) **Convex ensemble weighting** (`convex_max_sharpe`, `convex_target_vol` modes added to lib/ensemble_curve.py): 5-leg portfolio walk-forward, Calmar 1.04 vs baseline 0.79 — MDD reduction at CAGR cost. EOD-B IR-hyst SOLO still beats every ensemble. (3) **Quiet-flip filter** (entry_flip_skip_threshold engine param): post-hoc sim suggested Calmar 1.71, real engine result 1.22-1.37 — does NOT improve baseline. Post-hoc overestimates because slot-filling by next-ranked candidate is unaccounted for. **Conclusion: EOD-B IR-hyst at 22.26%/-16.49%/Sharpe 1.505/Calmar 1.350 is at the optimization frontier. -16.5% MDD is structural cost; cannot be reduced without proportional CAGR loss.** |
| 2026-05-01 | Intraday strategy search (NSE + US) + prop-firm system | `5c828fd`,`ed1d122`,`2cb2757`,`c5003d1` | **NSE intraday exhaustively dead:** open-low mean reversion (−22% CAGR), pair trading intraday (−6%), pair trading multi-day (−7% OOS). **US intraday:** QQQ/NAS100 gap-fade with SMA20 filter = Sharpe 0.70, WR 63%, CAGR +2.4% (2025: +9.5%). Deployed as BrightFunded prop-firm system ($100K account, ~$500/month target). Gold/EOD-momentum/RSI all dead. **Next:** DD analysis + minimization on EOD-B ensemble. |
| 2026-04-28 pt10 | Inspection drill: Phase 3 complete (3a-3f) | `34cb0d4`, `b738942`, `f3ae2b0` | **Phase 3a:** `scripts/fetch_ohlcv_window.py` — CLI/module wrapper around `engine/data_provider.py` for OHLCV spot-checks (default `nse_charting`). Smoke-test confirms event-row open matches audit's entry_price. **Phase 3b:** `scripts/audit_sanity_checks.py` — 8 cross-foot invariants, both champions pass all 8. Surfaces: eod_b's `simulator_trade_log.exit_reason` is uniformly `"natural"` (signal generator does not propagate exit_reason; audit hook is authoritative). **Phase 3c:** `scripts/spot_check_audits.py` — 20 stratified entries + 5 capacity-blocked per strategy with audit↔sim diff. 100% match on entries+exits at 1e-3 tolerance. **Phase 3d:** `scripts/pattern_queries.py` — 10 pattern queries IS/OOS split per strategy. Scanner pass-rate drift confirmed: ~14% (2010) → ~45% (2024). **Phase 3e+3f:** `docs/inspection/FINDINGS_eod_breakout.md`, `FINDINGS_eod_technical.md`, `FINDINGS_cross.md`. Headline findings: (a) BOTH strategies are 99%+ capacity-blocked — the position cap is the binding constraint, not the entry filter; (b) eod_t OOS open-position bias inflates headline pnl — realised OOS trailing_stop pnl is NEGATIVE (-0.18% mean, -8.4M total); (c) 2018 was the worst year for both strategies (Indian small/mid-cap correction); (d) OOS Direction-score distribution shifts upward in BOTH strategies (universe quality shift in 2025); (e) eod_t's `close > n_day_ma` clause is essentially redundant (conditional_fail_rate 0.13%) — Phase-4 hypothesis. Test suite 435/435 OK throughout. Handover: [`sessions/2026-04-28_pt10_handover.md`](sessions/2026-04-28_pt10_handover.md). |
| 2026-04-28 pt9 | Inspection drill: Phases 2d+2e complete (Phase 2 done) + review/fix | `9c635ff`, `3aac0fc`, `bbb0e06` | **Phase 2d:** `tests/test_audit_noninvasive.py` (9 tests) — static guards always run + slow runtime regression gated by `STRATEGY_BACKTESTER_AUDIT_REGRESSION=1`. 435/435 fast tests pass; 9/9 slow tests pass in 159s. **Phase 2e:** `scripts/run_audit_drill.py` runs both champions with `audit_mode=True` and writes 6 parquets per strategy (entry_audit, trade_log_audit, scanner_snapshot/scanner_reject_summary, filter_marginals, simulator_trade_log, equity_curve) + README + run_metadata.json under `results/<strategy>/audit_drill_<UTC-iso>/`. ~37s/strategy. **Findings already from artifacts:** (a) scanner_pass conditional_fail_rate **71.93%** (eod_b) / **69.45%** (eod_t) — scanner is the dominant entry constraint on both. (b) eod_t `clause_close_gt_ma` conditional_fail_rate **0.13%** — MA filter is essentially redundant given the n_day_high breakout, Phase-4 hypothesis. (c) eod_b `regime_flip` share jumps 22.9% IS → 42.6% OOS in 2025. (d) eod_t `end_of_data` jumps 0.16% → 5.7% in 2025 (positions still open at sim end). (e) Scanner pass-rate jumps 24.1% IS → 41.2% OOS — universe got more liquid relative to static thresholds. Phase 3 (inspection) next. Handover: [`sessions/2026-04-28_pt9_handover.md`](sessions/2026-04-28_pt9_handover.md). |
| 2026-04-28 pt8 | Inspection drill: Phase 2c complete | `813a89d` | **Phase 2c:** 5 audit hooks across `engine/scanner.py` + `engine/order_generator.py` (~95 lines, all gated). HOOK B per-clause reject summary in scanner; HOOK C clause mirror cols + HOOK D entry_audits emit + at-entry context capture in order_generator; HOOK F post-Pool join into trade_log_audits. eod_technical wrapper unchanged (context flows through). Smoke tests: eod_t `audit_mode=False` byte-identical to baseline (1303 trades / 5922 days); eod_t `audit_mode=True` (stub collector) ditto on trades+equity. eod_b re-verified byte-identical (defensive). Test suite 426/426. **Findings:** champion `n_day_gain_threshold:-999` empirically rejects 0 rows (confirms PIPELINE_MAP open question #1); eod_t exit_reason 98.5% TSL / 1.1% end_of_data / 0.4% anomalous_drop (no regime_flip — regime disabled in champion). Multiprocessing nondeterminism: natural diff passed without forcing `multiprocessing_workers=1` (confirms PIPELINE_MAP open question #2 — single-config champion is deterministic via `generate_order_df` sort). Phase 2d (regression test) next. Handover: [`sessions/2026-04-28_pt8_handover.md`](sessions/2026-04-28_pt8_handover.md). |
| 2026-04-28 pt7 | Inspection drill: Phase 2b complete | `f6ca734` | **Phase 2b:** 4 audit hooks added to `engine/signals/eod_breakout.py` (~70 lines, all gated by `audit_mode and audit_collector is not None`). HOOK 1 post-scanner snapshot, HOOK 2 per-clause flags + `all_clauses_pass`, HOOK 3 per-trade with at-entry context, HOOK 3a `_walk_forward_tsl` returns `(epoch, price, exit_reason)` always. Smoke tests: `audit_mode=False` byte-identical to `champion_pre_audit_baseline.json` (1795 trades, 5922 days); `audit_mode=True` (stub collector) ditto on trades+equity. Test suite 426/426. exit_reason distribution as expected (73.6% TSL / 26.2% regime_flip / 0.2% anomalous_drop). Independent sanity: `clause_scanner_pass` (23.62%) matches scanner_snapshot pass rate exactly. Phase 2c (eod_t legacy path) next. Handover: [`sessions/2026-04-28_pt7_handover.md`](sessions/2026-04-28_pt7_handover.md). |
| 2026-04-28 pt6 | Inspection drill: Phases 0+1+2a complete | `22efb42`,`2829a49`,`98d85da` | (1) **Phase 0:** both champions re-pinned at `3457840` engine state. eod_b CAGR 17.68% / Sharpe 1.183 (canonical confirmed); eod_t CAGR 19.63% / Sharpe 1.067. (2) **Phase 1:** [`docs/inspection/PIPELINE_MAP.md`](inspection/PIPELINE_MAP.md) — branch enumeration of 4 hook-target signal-side files; 3 non-trivial findings surfaced (n_day_gain redundancy under top_gainer, eod_t multiprocessing low-impact, eod_b missing exit_reason). (3) **Phase 2a:** `lib/audit_io.py` (652 lines) — 5 schemas + writers + helpers; smoke-tested; filter-marginal math validated. Phase 2b (hooks in `engine/signals/eod_breakout.py`) starts next session. Handover: [`sessions/2026-04-28_pt6_handover.md`](sessions/2026-04-28_pt6_handover.md). |
| 2026-04-28 pt3 | eod_b vol-filter test (negative) | `81381ee`,`3457840` | Added universe-level `max_stock_vol_pct` + `vol_lookback_days` entry params to eod_breakout (gated by sentinel; smoke-tested byte-identical for default config). 6-threshold sweep: every setting hurts both Sharpe (-0.30 to -0.40) and Calmar. Universe-level vol filtering is NOT the right lever for breakout strategies — high-vol stocks ARE the alpha contributors; MDD is regime-driven not stock-vol-driven. Writeup: [`strategies/eod_breakout/VOL_FILTER_2026-04-28.md`](../strategies/eod_breakout/VOL_FILTER_2026-04-28.md). Code retained (gated) for future experiments. |
| 2026-04-28 pt2 | N-leg ensemble + QDT R5 + R4c/R4d backfill + low_pe data check | | (1) Tested 3-leg eod_b+eod_t+low_pe. 2-leg champion stays (Sharpe 1.281). [`strategies/ensembles/N_LEG_EXPERIMENT_2026-04-28.md`](../strategies/ensembles/N_LEG_EXPERIMENT_2026-04-28.md). (2) QDT R5 refit (72 configs): new champion `tsl=10, ppi=2` gives Cal 0.388→0.451. (3) R4c run for all 6 backlog strategies + R4d for 2 price-only. [`R4C_R4D_BACKFILL_2026-04-28.md`](R4C_R4D_BACKFILL_2026-04-28.md). (4) low_pe pre-2018 data investigation: FMP coverage 1547-2126 symbols/year — **NOT sparse**. Pre-2018 cash gap is filter+scanner intersection issue. No data backfill needed. [`strategies/low_pe/PRE_2018_INVESTIGATION_2026-04-28.md`](../strategies/low_pe/PRE_2018_INVESTIGATION_2026-04-28.md). |
